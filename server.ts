...
async function startServer() {
  ...
  const CACHE_TTL = 300000; // 5 minute cache to respect rate limits
  
  // Persistent last known values for critical indices
  let lastKnownNifty = { value: "24,141.95", change: "-0.42%", trend: "down" };
  let lastKnownForex = { value: "84.38", change: "Real-time", trend: "up" };
        ...
        tasks.push((async () => {
          try {
            ...
            if (data) {
              const res = { 
                label: "NIFTY 50", 
                value: data.lastPrice || data.price || lastKnownNifty.value, 
                change: (data.pChange || data.change || lastKnownNifty.change) + "%", 
                trend: parseFloat(data.pChange || data.change || 0) >= 0 ? 'up' : 'down' 
              };
              lastKnownNifty = { value: res.value, change: res.change, trend: res.trend };
              results.push(res);
            }
          } catch (e: any) { 
            if (e.message === "RATE_LIMIT") {
               console.warn("LOG: IndianAPI rate limited, serving persistent buffer data");
               results.push({ label: "NIFTY 50", ...lastKnownNifty, status: 'STALE_BUFFER' });
            } else {
